...
export const parseNameAndPath = (rawInput: string) => {
  ...
  // If the user ran `npx create-pstack-app .` or similar, the appName should be the current directory
  if (appName === ".") {
  ...
  const p = paths.filter((p) => !p.startsWith("@")).join("/");

  return [appName, p] as const;
};
